...
def analyze_argmax_differences(ref_logits, test_logits):
    ...
    batch_size, seq_len = ref_logits.shape

    # Compute argmax along the vocabulary dimension (dim=2)
    ref_argmax = torch.argmax(ref_logits, dim=1)  # Shape: [batch_size, seq_len]
    test_argmax = torch.argmax(test_logits, dim=1)  # Shape: [batch_size, seq_len]
    ...
    for i in range(num_examples):
        b = diff_indices[i].tolist()
        ref_token = ref_argmax[b].item()
        test_token = test_argmax[b].item()
        examples.append({
            "batch": b,
...
def main():
    ...
    if argmax_stats['example_diffs']:
        ...
        for i, example in enumerate(argmax_stats['example_diffs']):
            print(
                f"  {i + 1}. ({example['batch']}: {example['ref_token_id']} vs {example['test_token_id']}")
